...
public class TicketService {
     ...
     * Use distributed lock for status changes that might have side effects
     */
    @Transactional
    ...
    private void validateStatusTransition(TicketStatus from, TicketStatus to) {
        ...
        // - Cannot go directly from OPEN to CLOSED without being RESOLVED
    ...
    /**
     * Unassign a ticket (remove assignee)
     */
    @Transactional
    public Ticket unassignTicket(UUID ticketId) {
        ...
        log.info("Attempting to unassign ticket {}", ticketId);
        ...
        try {
            ...
            try {
                ...
                if (ticket.getAssigneeId() == null) {
                    throw new InvalidTicketOperationException("Ticket is not assigned");
                }
